...
def build_script_source(
    ...
    if system_dependencies:
        ...
        script_parts.extend([
            ...
            "  # Use nix-portable with nix-shell to make packages available",
            "  # nix-shell downloads and makes packages available in the environment",
            "  set +e  # Temporarily disable exit on error to capture output",
            "  echo \"Installing $dep (this may take a few minutes on first run)...\"",
            "  # Use nix-shell -p to download and prepare the package",
            "  # This ensures packages are stored in the shared store when NP_STORE is set",
            "  # Timeout after 10 minutes to prevent hanging",
            "  INSTALL_OUTPUT=$(timeout 600 nix-portable nix-shell -p $dep --run 'echo \"Package $dep ready\"' 2>&1)",
            "  INSTALL_STATUS=$?",
            "  set -e  # Re-enable exit on error",
            "  if [ $INSTALL_STATUS -eq 0 ]; then",
            "    # Analyze output to determine cache hit/miss",
            "    if echo \"$INSTALL_OUTPUT\" | grep -q \"will be fetched.*download\"; then",
            "      DOWNLOAD_SIZE=$(echo \"$INSTALL_OUTPUT\" | grep -oP '\\d+\\.\\d+ MiB download' | head -1 || echo \"unknown\")",
            "      if echo \"$DOWNLOAD_SIZE\" | grep -qE '^0\\.0|^0 '; then",
            "        echo -e \"\\033[0;34m[NIX CACHE]\\033[0m \\033[0;32m✓\\033[0m $dep from \\033[1;32mLOCAL CACHE\\033[0m (0 MiB download)\"",
            "      else",
            "        echo -e \"\\033[0;34m[NIX CACHE]\\033[0m \\033[0;31m✗\\033[0m $dep from \\033[1;31mEXTERNAL\\033[0m: $DOWNLOAD_SIZE\"",
            "      fi",
            "    elif echo \"$INSTALL_OUTPUT\" | grep -q \"copying path.*from.*cache.nixos.org\"; then",
            "      echo -e \"\\033[0;34m[NIX CACHE]\\033[0m \\033[0;31m✗\\033[0m $dep from \\033[1;31mEXTERNAL\\033[0m (cache.nixos.org)\"",
            "    else",
            "      echo -e \"\\033[0;34m[NIX CACHE]\\033[0m \\033[0;32m✓\\033[0m $dep from \\033[1;32mLOCAL CACHE\\033[0m\"",
            "    fi",
            "    echo \"✓ $dep prepared successfully\"",
            "    # Copy packages to shared store if NP_STORE is set",
            "    if [ -n \"$NP_STORE\" ] && [ -d \"$NP_STORE\" ] && [ -d ~/.nix-portable/nix/store ]; then",
            "      echo \"Copying packages to shared store...\"",
            "      # Copy any new packages from nix-portable's store to shared store",
            ...
            "    fi",
            "  else",
            "    echo -e \"\\033[0;31m[ERROR]\\033[0m Failed to prepare $dep\"",
            "    echo \"Error output:\"",
            "    echo \"$INSTALL_OUTPUT\" | tail -10",
            "    echo \"Warning: $dep may not be available, continuing...\"",
            "  fi",
            "done",
            "",
            "# Build nix-shell command with all system dependencies",
            "if [ -n \"$SYSTEM_DEPS\" ]; then",
            "  # Convert to nix-shell format: -p pkg1 -p pkg2 ...",
            "  NIX_PACKAGES=\"\"",
            "  for dep in $SYSTEM_DEPS; do",
            "    NIX_PACKAGES=\"$NIX_PACKAGES -p $dep\"",
            "  done",
            "  export NIX_SHELL_PACKAGES=\"$NIX_PACKAGES\"",
            "  echo \"System dependencies will be available via nix-shell: $NIX_PACKAGES\"",
            "fi",
            "",
            "# Final cache status",
            "if [ -n \"$NP_STORE\" ] && [ -d \"$NP_STORE\" ]; then",
