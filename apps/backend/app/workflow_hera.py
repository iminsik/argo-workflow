...
def build_script_source(
    ...
    script_parts.extend([
        ...
        "  set +e",
        "  NIX_OUTPUT=$(nix-portable nix-shell $NIX_SHELL_PACKAGES --run 'python -c \"$PYTHON_CODE\"' 2>&1)",
        "  NIX_EXIT=$?",
        "  set -e",
        "  ",
        ...
        "    rsync -a --link-dest=~/.nix-portable/nix/store ~/.nix-portable/nix/store/ \"$NP_STORE/\" 2>/dev/null || {",
