...
def build_step_script_source(
    ...
    if system_dependencies:
        ...
        if use_cache:
            script_parts.extend([
                "# Mount PVC directly to nix-portable locations using symlinks",
                "# This eliminates the need for rsync - nix-portable uses the PVC directly",
                ...
                "fi",
                "  ",
                "# IMPORTANT: nix-portable needs to initialize its own store structure first",
                "# We'll use its default location initially, then sync to shared PVC",
                "# This avoids initialization issues when symlinking to an empty/pre-populated directory",
                "mkdir -p ~/.nix-portable/nix",
                "mkdir -p ~/.nix-portable/nix/var/nix/db",
                "  ",
                "# Check if database exists in shared store - if so, copy it first",
                "if [ -f /nix/store/.nix-db/db.sqlite ]; then",
                "  echo \"Found existing database in shared store, copying to nix-portable location...\"",
                "  cp /nix/store/.nix-db/db.sqlite ~/.nix-portable/nix/var/nix/db/db.sqlite",
                "  echo \"Database copied successfully\"",
                "fi",
                "  ",
                "# Copy packages from shared store to nix-portable default location",
                "# This allows nix-portable to recognize them via the database",
                "if [ -d /nix/store ] && [ \"$(ls -A /nix/store 2>/dev/null)\" ]; then",
                "  echo \"Copying packages from shared store to nix-portable location...\"",
                "  mkdir -p ~/.nix-portable/nix/store",
                "  ",
                "  # Try rsync first (more efficient with hardlinks)",
                "  if command -v rsync >/dev/null 2>&1; then",
                "    RSYNC_OUTPUT=$(rsync -a --link-dest=/nix/store /nix/store/ ~/.nix-portable/nix/store/ 2>&1)",
                "    RSYNC_EXIT=$?",
                "    if [ $RSYNC_EXIT -eq 0 ]; then",
                "      echo \"Packages copied via rsync (using hardlinks for efficiency)\"",
                "    else",
                "      echo \"rsync failed (exit $RSYNC_EXIT), trying cp...\"",
                "      echo \"rsync error: $RSYNC_OUTPUT\"",
                "      cp -r /nix/store/* ~/.nix-portable/nix/store/ 2>/dev/null || true",
                "      echo \"Packages copied via cp\"",
                "    fi",
                "  else",
                ...
                "  echo -e \"\\033[0;34m[NIX CACHE]\\033[0m Copied $PACKAGE_COUNT packages to nix-portable store\"",
                "else",
                "  echo \"No packages found in shared store (first run)\"",
                "fi",
                "  ",
    ...
    script_parts.extend([
        ...
        "  fi",
        "  ",
        "  # Sync packages and database back to shared PVC after execution",
        "  # This ensures the cache is updated for subsequent runs",
        "  if [ -n \"$NP_STORE\" ] && [ -d \"$NP_STORE\" ]; then",
        "    echo \"Syncing packages and database back to shared store...\"",
        "    ",
        "    # Sync packages (use rsync with link-dest for efficiency)",
        "    if command -v rsync >/dev/null 2>&1; then",
        "      RSYNC_OUTPUT=$(rsync -a --link-dest=~/.nix-portable/nix/store ~/.nix-portable/nix/store/ \"$NP_STORE/\" 2>&1)",
        "      RSYNC_EXIT=$?",
        "      if [ $RSYNC_EXIT -eq 0 ]; then",
        "        echo \"Packages synced via rsync (using hardlinks for efficiency)\"",
        "      else",
        "        echo \"rsync failed (exit $RSYNC_EXIT), trying cp...\"",
        "        echo \"rsync error: $RSYNC_OUTPUT\"",
        "        cp -r ~/.nix-portable/nix/store/* \"$NP_STORE/\" 2>/dev/null || true",
        "        echo \"Packages synced via cp\"",
        "      fi",
        "    else",
        "      echo \"rsync not available, using cp...\"",
        "      cp -r ~/.nix-portable/nix/store/* \"$NP_STORE/\" 2>/dev/null || true",
        ...
        "    # Sync database",
        "    if [ -f ~/.nix-portable/nix/var/nix/db/db.sqlite ]; then",
        "      mkdir -p \"$NP_STORE/.nix-db\"",
        "      cp ~/.nix-portable/nix/var/nix/db/db.sqlite \"$NP_STORE/.nix-db/db.sqlite\"",
        "      DB_SIZE=$(stat -c%s \"$NP_STORE/.nix-db/db.sqlite\" 2>/dev/null || echo 0)",
        "      echo -e \"\\033[0;34m[NIX CACHE]\\033[0m Database synced to shared store (size: $DB_SIZE bytes)\"",
        "    fi",
        "    ",
        "    PACKAGE_COUNT=$(find \"$NP_STORE\" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | wc -l)",
        "    echo -e \"\\033[0;34m[NIX CACHE]\\033[0m Packages in shared store: $PACKAGE_COUNT\"",
        "  fi",
        "else",
